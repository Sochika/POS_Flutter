...
class _TransactionListScreenState extends State<TransactionListScreen> {
  ...
  void initState() {
    if(widget.fromCustomer){
      Get.find<TransactionController>().getCustomerWiseTransactionListList(widget.customerId, 1, isUpdate: false);
    }else{
            ...
            slivers: [
              SliverToBoxAdapter(
                child: Column(children: [
                  CustomHeader(title: 'transaction_List'.tr , headerImage: Images.peopleIcon),
                                    ...
                                    Container(
                                      ...
                                      child: DropdownButton<int>(
                                        ...
                                        items: transactionController.transactionTypeIds.map((int? value) {
                                          return DropdownMenuItem<int>(
                                              value: value,
                                              child: Text(transactionController.transactionTypeList![(transactionController.transactionTypeIds.indexOf(value))].tranType!));
                                        }).toList(),
                                        onChanged: (int? value) {
                                  ...
                                  onTap: (){
                                    ...
                                    if(startDate == 'null'  && endDate == 'null'){
                                      ...
                                      transactionController.getTransactionFilter(selectedStartDate, selectedEndDate,
                                          Get.find<AccountController>().accountIndex, transactionController.transactionTypeName ?? 'income',1 );
                                    }
                  ...
                  widget.fromCustomer ? CustomerTransactionListView(
                    isHome: false,scrollController: _scrollController,
                    customerId: widget.customerId,
                  ) : const TransactionListView(isHome: false,)
