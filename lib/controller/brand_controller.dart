...
class BrandController extends GetxController implements GetxService{
  ...
  Future<http.StreamedResponse> addBrand(String brandName, int? brandId) async {
    ...
    if(response.statusCode == 200) {
      ...
      _isLoading = false;
      Get.back();
      showCustomSnackBar(brandId != null ? 'brand_updated_successfully'.tr : 'brand_added_successfully'.tr, isError: false);
      _brandImage = null;
  ...
  Future<void> getBrandList(int offset, {Products? product, bool reload = false, int limit = 10}) async {
    if(reload){
      _brandList = [];
    }
    _isLoading = true;
