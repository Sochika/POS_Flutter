...
class CategoryController extends GetxController implements GetxService{
  ...
  Future<void> getCategoryList(int offset, {Products? product, reload = false, int limit = 10}) async {
    if(reload){
      _categoryList =[];
    }
    _isGetting = true;
  ...
  Future<void> getSubCategoryList(int offset, int? categoryId, {Products? product, bool reload= false, bool isUpdate = true}) async {
      ...
      if(product != null && product.categoryIds != null){
        for(int i = 0; i < product.categoryIds!.length; i++) {
          if(product.categoryIds?[i].position == 2) {
            setSubCategoryIndex(int.parse('${product.categoryIds![i].id}'), false);
          }
        }
      }
      _isGetting = false;
